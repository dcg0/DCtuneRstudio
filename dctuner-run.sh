...
export DCTUNER_PORT DCTUNER_BAUD DCTUNER_WEB_PORT DCTUNER_HOST=0.0.0.0
...
if [ "$DCTUNER_SERIE" = "1" ]; then
  exec python3 servidor.py --host 0.0.0.0 --port "$DCTUNER_WEB_PORT" --serie
else
  exec python3 servidor.py --host 0.0.0.0 --port "$DCTUNER_WEB_PORT"
fi
