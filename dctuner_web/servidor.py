...
probar la interfaz sin enviar comandos al vehículo.
"""
...
except ImportError:  # El instalador ofrece python3-serial; el simulador sigue disponible.
    serial = None
...
class ECUTransport:
    ...
    def start(self) -> None:
        self._stop.clear()
        target = self._simulation_loop if self._simulation else self._serial_loop
    ...
    def _simulation_loop(self) -> None:
        ...
        while not self._stop.is_set():
            tick += 1
            phase = tick / 18.0
            rpm = 950 + (math.sin(phase) * 650) + (math.sin(phase / 3) * 190)
            item = Telemetry(timestamp=time.time(), rpm=max(0, rpm), temperature=86 + math.sin(phase / 5) * 3, pressure=98 + math.sin(phase) * 11, afr=14.6 + math.sin(phase * 1.4) * 0.45, load=32 + math.sin(phase / 2) * 14, advance=12 + math.sin(phase * 0.7) * 4, voltage=13.8 + math.sin(phase / 4) * 0.12, throttle=18 + math.sin(phase) * 9, status="simulated", source="simulador")
    ...
    def send_safe_command(self, command: str) -> tuple[bool, str]:
        ...
        if self._simulation:
            return False, "El modo simulación no envía comandos a la ECU"
        if not self._serial:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Servidor web local DC TUNER STUDIO")
    parser.add_argument("--host", default=WEB_HOST)
    parser.add_argument("--port", type=int, default=WEB_PORT)
    parser.add_argument("--serie", action="store_true", help="inicia usando el puerto serie configurado")
    parser.add_argument("--self-test", action="store_true")
    args = parser.parse_args()
    if args.self_test:
        raise SystemExit(self_test())
    transport._simulation = not args.serie
    transport.start()
    log.info("DC TUNER STUDIO disponible en http://%s:%s (modo %s)", args.host, args.port, "serie" if args.serie else "simulación")
